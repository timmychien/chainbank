        ...
        pool.getConnection(function (err, connection) {
            connection.query('SELECT * FROM user_information WHERE user_id = ?', [req.session.user_id], function (err, rows) {
                if (err) {
                    ...
                    res.render('makeup/makeuppayment',{user:req.session.user_id});
                }
        ...
        pool.getConnection(function (err, connection) {
            connection.query('SELECT * FROM credit WHERE borrower_id = ? AND id = ? ', [req.session.user_id,req.params.id], function (err, rows) {
                if (err) {
                    ...
                    res.render('makeup/makeup_id',{user:req.session.user_id,credit:req.params.id});
                }
            ...
            connection.query('SELECT * FROM credit WHERE borrower_id = ? AND id = ? ', [req.session.user_id,req.params.id], function (err, rows) {
                if (err) {
                    ...
                    connection.query('SELECT * FROM user_information WHERE user_id = ?', [req.session.user_id], function (err, rows) {
                        ...
                        res.render('makeup/makeupconfirm',{user:req.session.user_id,credit:req.params.id,numoftokens:num_of_tokens,not_pay:not_pay-num_of_tokens});
                        }
                    })
                }
