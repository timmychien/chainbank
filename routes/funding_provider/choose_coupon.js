...
router.get('/', function (req, res) {
    res.render('funding_requirement/choose_coupon',{
...
router.post('/',function(req,res){
    ...
    var invest_amount=req.body['invest_amount'];
    var coupon_class=req.body['coupon_class'];
    var total_class=3;
    if(coupon_class==1){
        var coupon_rate=5;
    }else if(coupon_class==2){
        var coupon_rate=2;
    }else{
        coupon_rate=1;
    }
    ...
    pool.connection(function(err,connection){
        connection.query('SELECT * FROM credit WHERE requirementID=?',[requirementID],function(err,rows){
...
router.post('/', function (req, res) {
    ...
    console.log('amount_to_invest:', invest_amount)

    var investor = req.session.user_wallet_address;
    pool.getConnection(function (err, connection) {
        connection.query('SELECT * FROM funding_requirement WHERE creditID=?', [creditID], function (err, rows) {
            if (err) {
                res.render('error', {
                    message: err.message,
                    error: err
