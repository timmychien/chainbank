window.addEventListener('load', () => {
  ...
  let text_sheet=document.getElementById("invest_details");
  
  fetch('/getuserinvestment/'+user.replace("會員中心",""))
      .then(res=>res.json())
      .then(data=>{
      console.log(data);        
      for(var i=0;i<data.length;i++){
          ...
          per_credit.setAttribute("style","padding-right:15px;padding-left:25px;justify-content:space-between;display:flex;padding-bottom:20px;font-size:20px;font-weight:lighter;padding-top:20px;color:#444040;border-bottom:solid #f7f7f7 0.05px;background-color: white;");
          creditid.setAttribute("style","width:90px;");
          borrow_time.setAttribute("style","width:100px;");
          deadline.setAttribute("style","width:150px;");
          ...
          document.getElementById("creditid_"+(i+1)).innerHTML=data[i].com_name;    
          var type =data[i].requirementID_code.slice(1,2);
          if(type=='M'){
            document.getElementById("credit_not_pay_"+(i+1)).innerHTML='保證金';              
          }else if(type=='C'){
            document.getElementById("credit_not_pay_"+(i+1)).innerHTML='擔保品';               
          }
  
          document.getElementById("credit_borrow_time_"+(i+1)).innerHTML=data[i].updated_price.toFixed(4);
          document.getElementById("credit_deadline_"+(i+1)).innerHTML=data[i].keep_rate.toFixed(4);      
        }
      });
  ...
  setInterval(()=>{update_investment()},10000);